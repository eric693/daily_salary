...
async function calculateSalary() {
    ...
    const calculationData = {
        ...
        saturdayPay: Number(currentEmployeeData.saturdayWage) || 0,  // ⭐ 直接使用員工設定的週六出工金額
        ...
        saturdayWage: Number(currentEmployeeData.saturdayWage) || 0,
    ...
    console.log('   週六出工:', calculationData.saturdayPay);
...
function calculateLocalSalary(data) {
    ...
    const saturdayPay = currentEmployeeData.saturdayWage || 0;  // ⭐ 週六出工直接使用員工設定值
    ...
    // ⭐ 週六出工加入津貼總額
    const totalAllowance = saturdayPay + mealTotal + attendanceAllowance + jobAllowance + rentAllowance + advanceAllowance;
    ...
    return {
        ...
        saturdayPay: saturdayPay,  // ⭐ 新增週六出工
...
function displayResult(result) {
    ...
    // ⭐ 週六出工顯示在加項津貼明細中
    document.getElementById('resultSaturdayPay').textContent = `NT$ ${result.saturdayPay.toLocaleString()}`;
